...
async def get_all(db: Session) -> Awaitable[Optional[List[Product]]]:
    return db.query(Product).all()

async def get_by_id(product_id: UUID, db: Session) -> Awaitable[Optional[Product]] :
    return db.query(Product).filter_by(id=product_id).one()

async def insert_product(product: ProductSchema, db: Session) -> Awaitable[Product]:
...
async def update_product(product: UpdateProductSchema, db: Session) -> Awaitable[UpdateProductSchema]:
...
async def delete_product(product_id: UUID, db: Session) -> Awaitable[DeleteProductResponse]:
